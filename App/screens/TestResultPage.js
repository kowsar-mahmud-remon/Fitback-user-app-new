...
import { Dimensions, FlatList, ActivityIndicator, StyleSheet, SafeAreaView, View, StatusBar, Pressable, Text, ScrollView, TextInput, Image, Linking, TouchableOpacity, Alert, Animated } from 'react-native';
...
function TestResultPage({ navigation, route }) {
...
/>
  ...
  const navigationThree = useNavigation();
  ...
  const loadData = async () => {
    try {
      const title = await AsyncStorage.getItem("currentTitle");
      const correctAnswers = await AsyncStorage.getItem("correctAnswersCount");
      const totalQuestionsNumber = await AsyncStorage.getItem("totalQuestions");
      const passOrFailDetails = await AsyncStorage.getItem("passOrFail");
      const allWrongAnswers = await AsyncStorage.getItem("wrongAnswers");
      const fCoin = await AsyncStorage.getItem("fCoin");

      if (title !== null) {
        setCurrentTitle(JSON.parse(title));
        setCorrectAnswersCount(JSON.parse(correctAnswers));
        setTotalQuestions(JSON.parse(totalQuestionsNumber));
        setPassOrFail(JSON.parse(passOrFailDetails));
        setWrongAnswers(JSON.parse(allWrongAnswers));
        setFCoin(JSON.parse(fCoin));
      }
    } catch (error) {
      ...
      <View style={styles.MainContainer}>
        ...
        <View style={[styles.navbar, { flexDirection: 'row' }]}>
          ...
          <View style={{ width: '50%', justifyContent: 'flex-start', alignItems: 'flex-start', }}>
            ...
            <Text style={{ top: 17, color: colors.black, fontSize: 14, display: lan ? 'flex' : 'none', letterSpacing: .9, fontFamily: 'Poppins_400Regular' }} >  Quiz Result </Text>
          ...
          <View style={{ width: '100%', justifyContent: 'center', alignItems: 'center', marginTop: 40, paddingHorizontal: 10 }}>
            <View style={{ width: '100%', marginTop: 20 }}>
              <View style={{
                ...
                <Image
                  source={require("../assets/fitback/resultImg.png")}
                  style={{
                    width: "100%",
                    height: 200,
                    position: "absolute",
                ...
                <Text style={{
                  ...
                  {fCoin == 0 ? 0 : fCoin} Coin
                </Text>
              </View>

              <View style={styles.mainContent}>
                <View style={styles.resultContainer}>
                  <View style={styles.resultContentOne}>
                    <Text style={styles.resultTextOne}>
                      <Text style={styles.questions}>{totalQuestions} </Text>
                      Questions
                    </Text>
                    <Text style={styles.resultTextTwo}>
                      <Text style={styles.correct}>{correctAnswersCount} </Text>
                      Correct
                    </Text>
                  </View>

                  <View style={styles.resultContentTwo}>
                    <Text style={styles.resultTextOne}>
                      <Text style={styles.unanswered}>00 </Text>
                      Unanswered
                    </Text>
                    <Text style={styles.resultTextTwo}>
                      <Text style={styles.incorrect}>{wrongAnswers?.length} </Text>
                      Incorrect
                    </Text>
                  </View>
                </View>

                {/* <View style={styles.ReAttempt}>
                <Image
                  source={require("../../assets/ReAttemptImg.png")}
                  style={styles.ReAttemptImg}
                />
                <View style={styles.ReAttemptContent}>
                  <Text style={styles.ReAttemptText}>Boost Your Performance</Text>


                  <TouchableOpacity
                    onPress={() =>
                      navigationTwo.navigate("Practice", {
                        defaultActiveArray: "retryRequired",
                      })
                    }
                  >
                    <Text style={styles.ReAttemptBtn}>Re-Attempt</Text>
                  </TouchableOpacity>
                </View>
              </View> */}
                ...
                <View style={styles.suggestContent}>
                  <Text style={styles.suggestTitle}>Not bad</Text>
                  <Text style={styles.suggestText}>
                    We understand that the Test was not very easy but we believe
                    that you can definitely do better!
                  </Text>
                </View>
                ...
                <View style={styles.resultButtonContainer}>
                  <TouchableOpacity
                    onPress={() =>
                      navigationTwo.navigate("IncorrectAnswerPage", {
                        currentTitle: "wrongAnswers",
                        wrongAnswers,
                      })
                    }
                  >
                    <Text style={styles.resultButton}>Incorrect Answer</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    onPress={() =>
                      navigationTwo.navigate("FullSolutionsPage", {
                        currentTitle,
                      })
                    }
                  >
                    <Text style={styles.resultButton}>Full Solutions</Text>
                  </TouchableOpacity>
                </View>
              </View>
