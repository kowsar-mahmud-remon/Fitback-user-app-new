...
function AddReview({ navigation, route }) {
  ...
  const handleSubmit = async () => {
    ...
    fetch('https://qwikit1.pythonanywhere.com/userReviews/new', requestOptions)
      .then((response) => {
        if (!response.ok) {
      ...
      <View style={styles.MainContainer}>
        ...
        <View style={[styles.navbar, { flexDirection: 'row' }]}>
          ...
          <View style={{ width: '50%', justifyContent: 'flex-start', alignItems: 'flex-start', left: 10 }}>
            ...
            <Text style={{ left: 10, top: 17, color: colors.black, fontSize: 14, display: lan ? 'flex' : 'none', letterSpacing: .9, fontFamily: 'Poppins_400Regular' }} >  Add Review </Text>
        ...
        <View style={[styles.body1, { justifyContent: 'flex-start', alignItems: 'center' }]}>
          <ScrollView style={{ width: "100%" }}>
            ...
            <View style={{ width: '100%', marginTop: 20, paddingHorizontal: 14 }}>
              ...
              <TouchableOpacity style={styles.button} onPress={handleSubmit} disabled={loadingTwo}>
                {loadingTwo ? (
