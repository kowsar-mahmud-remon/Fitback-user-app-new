...
function MyReport({ navigation, route }) {
      ...
      <View style={styles.MainContainer}>
        ...
        <View style={[styles.navbar, { flexDirection: 'row' }]}>
          ...
          <View style={{ width: '50%', justifyContent: 'flex-start', alignItems: 'flex-start', left: 10 }}>
            ...
            <Text style={{ left: 10, top: 17, color: colors.black, fontSize: 14, display: lan ? 'flex' : 'none', letterSpacing: .9, fontFamily: 'Poppins_400Regular' }} >  My Report </Text>
        ...
        <View style={[styles.body1, { justifyContent: 'flex-start', alignItems: 'center' }]}>
          <ScrollView style={{ width: "100%" }}>
            ...
            <View style={{ width: '100%', marginTop: 20, paddingHorizontal: 14 }}>
              <Text style={{ fontSize: 14, fontFamily: "Poppins_500Mediums", textAlign: "left", color: "#0F0F0F" }}>Your Report</Text>

              <View style={{ width: "100%", marginTop: 8 }}>
                {
                  userDetails?.report?.map(details => {
                    const fileName = details.report_file.split('/').pop();

                    const date = new Date(details.systemdate);
                    const formattedDate = `${date.getDate()}, ${date.toLocaleString('en-US', { month: 'short' })}, ${date.getFullYear()}`;

                    return (
                      <View key={details.id} style={{ width: "100%", flexDirection: "row", borderWidth: 1, borderColor: "#595959", marginBottom: 12, padding: 8, paddingRight: 0, borderRadius: 8 }}>
                        <View style={{ width: "83%" }}>
                          <Text style={{ fontSize: 12, fontFamily: "Poppins_500Medium", color: "#0F0F0F" }}>{fileName}</Text>
                          <Text style={{ fontSize: 8, fontFamily: "Poppins_500Medium", color: "#595959" }}>{formattedDate}</Text>
                        </View>


                        <View style={{ width: "15%", alignItems: "center", justifyContent: "center", borderLeftWidth: 1, borderColor: "#595959" }}>

                          <TouchableOpacity style={{}} onPress={() => handleDownload(details.report_file)}>
                            <Feather name="download" size={24} color="#595959" />
                          </TouchableOpacity>
                        </View>
                      </View>
                    );
              ...
              <View style={{ width: '100%', justifyContent: 'center', alignItems: 'center', marginTop: 20 }}>
                <View style={styles.tableContainer}>
                  <Text style={{ fontSize: 14, fontFamily: "Poppins_500Mediums", textAlign: "left", color: "#0F0F0F" }}>Session Report :</Text>
                  <View style={styles.table}>
                    <View style={styles.tableRow}>
                      <Text style={styles.tableHeading}>Session</Text>
                      <Text style={styles.tableHeading}>Weight Loss</Text>
                      <Text style={styles.tableHeading}>Belly Size Loss</Text>
                      <Text style={styles.tableHeading}>Service Taken</Text>
                      <Text style={styles.tableHeading}>Diet pattern</Text>
                    </View>
                    {userDetails?.sessionReport?.map((details, index) => {
                      const date = new Date(details.timestamp);
                      const formattedDate = `${date.getDate()}, ${date.toLocaleString('en-US', { month: 'short' })}, ${date.getFullYear()}`;
                      return (
                        <View key={index} style={styles.tableRow}>
                          {/* <Text style={styles.tableCell}>{details.sessionReportNo} {formattedDate}</Text> */}
                          <View style={styles.tableCell}>
                            <Text style={{
                              fontSize: 10,
                              fontFamily: 'Poppins_400Regular',
                              textAlign: "center",
                            }}>{details.sessionReportNo}</Text>
                            <Text style={{
                              fontSize: 10,
                              fontFamily: 'Poppins_400Regular',
                              textAlign: "center",
                          ...
                          <Text style={styles.tableCell}>{details.weightLoss}</Text>
                          <Text style={styles.tableCell}>{details.bellySizeReducation}</Text>
                          <Text style={styles.tableCell}>{details.descriptionOfServiceTaken}</Text>
                          <Text style={styles.tableCell}>{details.dietPattern}</Text>
                        </View>
                      );
                    })}
                  </View>
              ...
              <View style={{ width: '100%', justifyContent: 'center', alignItems: 'center', marginTop: 20 }}>
                <View style={styles.tableContainer}>
                  <Text style={{ fontSize: 14, fontFamily: "Poppins_500Mediums", textAlign: "left", color: "#0F0F0F" }}>Detox Report :</Text>
                  <View style={styles.table}>
                    <View style={styles.tableRow}>
                      <Text style={styles.tableHeading}>Detox</Text>
                      <Text style={styles.tableHeading}>Started Weight</Text>
                      <Text style={styles.tableHeading}>Current Weight</Text>
                      <Text style={styles.tableHeading}>Weight Loss</Text>
                      <Text style={styles.tableHeading}>Summary</Text>
                    </View>
                    {userDetails?.detoxReport?.map((details, index) => {
                      const date = new Date(details.timestamp);
                      const formattedDate = `${date.getDate()}, ${date.toLocaleString('en-US', { month: 'short' })}, ${date.getFullYear()}`;
                      return (
                        <View key={index} style={styles.tableRow}>
                          ...
                          <Text style={styles.tableCell}>{details.initialWeight}</Text>
                          <Text style={styles.tableCell}>{details.currentWeight}</Text>
                          <Text style={styles.tableCell}>{details.weightReduction}</Text>
                          <Text style={styles.tableCell}>{details.patientSummary}</Text>
                        </View>
                      );
                    })}
                  </View>
              ...
              <View style={{ width: '100%', justifyContent: 'center', alignItems: 'center', marginTop: 20 }}>
                <View style={styles.tableContainer}>
                  <Text style={{ fontSize: 14, fontFamily: "Poppins_500Mediums", textAlign: "left", color: "#0F0F0F" }}>Body Measurements Tracking with Reset Option :</Text>
                  <View style={styles.table}>
                    <View style={styles.tableRow}>
                      <Text style={styles.tableHeading}>Service Taken</Text>
                      <Text style={styles.tableHeading}>Previos Measurement</Text>
                      <Text style={styles.tableHeading}>Current Measurement</Text>
                      <Text style={styles.tableHeading}>Treatment Area</Text>
                    </View>
                    {userDetails?.bodyMeasurementsTracking?.map((details, index) => {
                      const date = new Date(details.timestamp);
                      const formattedDate = `${date.getDate()}, ${date.toLocaleString('en-US', { month: 'short' })}, ${date.getFullYear()}`;
                      return (
                        <View key={index} style={styles.tableRow}>
                          ...
                          <Text style={styles.tableCell}>{details.previousMeasurement}</Text>
                          <Text style={styles.tableCell}>{details.currentMeasurement}</Text>
                          <Text style={styles.tableCell}>{details.treatmentArea}</Text>
                        </View>
                      );
                    })}
                  </View>
