...
function IncorrectAnswerPage({ navigation, route }) {
  ...
  ////Incorrect Answer/////
  /////////////////
  const testTitle = route.params.currentTitle;
  const TotalWrongAnswers = route.params.wrongAnswers;
  const [allQuestions, setAllQuestions] = useState();

  const loadData = async () => {
    try {
      const allData = await AsyncStorage.getItem(testTitle);

      if (allData !== null) {
        setAllQuestions(JSON.parse(allData));
      }
    } catch (error) {
      ...
      <View style={styles.MainContainer}>
        ...
        <View style={[styles.navbar, { flexDirection: 'row' }]}>
          ...
          <View style={{ width: '50%', justifyContent: 'flex-start', alignItems: 'flex-start', left: 10 }}>
            ...
            <Text style={{ left: 10, top: 17, color: colors.black, fontSize: 14, display: lan ? 'flex' : 'none', letterSpacing: .9, fontFamily: 'Poppins_400Regular' }} > Incorrect Answer </Text>
        ...
        <View style={[styles.body1, { justifyContent: 'flex-start', alignItems: 'center' }]}>
          <ScrollView >
            ...
            <View style={{ width: '100%', justifyContent: 'center', alignItems: 'center', marginTop: 10, paddingHorizontal: 10 }}>

              <View style={styles.mainContainer}>
                <Text style={styles.mainTitle}>
                  <Text style={styles.mainTitleTwo}>{TotalWrongAnswers?.length} </Text>
                  Incorrect
                </Text>
                <View style={styles.container}>
                  {allQuestions?.map((data, i) => (
                    <View style={styles.mainContent} key={data?.id}>
